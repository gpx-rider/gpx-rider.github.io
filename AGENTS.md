...
| `app/flyover.mjs` | Pure animated-overview math (tested): orbit turntable camera |
...
- **Camera debug overlay.** A `"debug"` settings toggle
  ...
  `state.map.{tilt,range,heading,roll,center}` plus the derived eye altitude and
...
- **Overview motion modes** (`state.overviewMode`, a persisted user setting in
  ...
  and pitches down by `mountPitchDegrees`; `speedMps`, `flyHeightMeters`,
  `viewDistanceMeters`, `ellipseScale`, `minSemiMajorMeters`,
  `minSemiMinorMeters`, `sampleCount`, and `startAngleDegrees` are all tunable.
  `maxBankDegrees` scales the bank angle from the current turn radius
